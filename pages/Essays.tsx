...
import { MOCK_ESSAYS } from '../lib/constants';
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
                ...
                <div className="md:col-span-8">
                  <div className="flex items-center gap-4 text-xs font-nav text-mustard-500 uppercase tracking-widest mb-4">
                    <span className="flex items-center gap-1"><Calendar className="w-3 h-3" /> {new Date(essay.published_at!).toLocaleDateString()}</span>
                    <span className="w-1 h-1 rounded-full bg-white/20"></span>
                    <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {essay.read_time_minutes} min read</span>
                  </div>
